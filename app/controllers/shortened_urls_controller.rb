...
class ShortenedUrlsController < ApplicationController
  before_action :authorize_request, except: :redirect_to_original_url
  ...
  def redirect_to_original_url
    shortened_url = ShortenedUrl.find_by(shortened_url: params[:short_url])
    if shortened_url.present? && shortened_url.not_expired?
  ...
  def create
    ...
    if shortened_url.save
      ...
      render json: { message: 'Shortened URL was not created.' }, status: :unprocessable_entity
    end
  end
